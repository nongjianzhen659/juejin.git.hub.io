<template>
  <van-list
    ...
    @load="onLoad"
  >
    <div class="content-box" v-for="(item,index) in restaurantsComputer" :key="index">
      <div class="boxtitle">
        <ul class="titletop">
...
export default {
  ...
  computed: {
    ...
    restaurantsComputer() {
      if (this.searchVlu.value === "全部") {
        return this.restaurants;
      }
      if (this.searchVlu.value) {
        //this.searchVlu.value为点击后的到的值,如果值存在
        return this.restaurants.filter(item => {
          // window.console.log(item);
          //筛选、item.art为类型
          if (item.art.indexOf(this.searchVlu.value) >= 0) {
            return item;
          }
        });
      } else {
